...
class LtlcrossRunner(object):
    ...
    def run_ltlcross(self, automata=True, check=False, timeout='1200'):
    ...
    def compute_timeouts(self):
        if "exit_status" not in self.cols:
            raise RuntimeError("exit status not stored")
        codes = self.values.exit_status
        return codes[codes == "timeout"].count()
